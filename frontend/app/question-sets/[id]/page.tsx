...
export default function QuestionSetHomePage() {
  ...
  useEffect(() => {
    if (data) {
      const maxPage = Math.max(0, Math.ceil(data.questions.length / PAGE_SIZE) - 1);
  ...
  const visibleQuestions = data.questions.slice(page * PAGE_SIZE, page * PAGE_SIZE + PAGE_SIZE);
    ...
    <>
      ...
      <main className="mx-auto max-w-2xl py-16">
        ...
        <div className="mt-6">
          ...
          <p className="mt-8 text-sm text-gray-400">질문을 클릭하면 답을 작성할 수 있어요</p>
        </div>
