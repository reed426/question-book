...
**일반 유저(demo 계정)로 확인할 흐름**: 로그인 → 질문팩 선택 → 모드/시작일 설정 → 홈/타임라인에서 질문 답변(텍스트+사진) → 북 미리보기 → 공유 링크 생성 → 로그아웃 없이 새 탭에서 공유 링크 열어보기.

**관리자(admin 계정) 확인 방법**: 일반 화면에는 관리자 페이지로 가는 메뉴가 따로 없습니다. 로그인 후 주소창에 직접 http://localhost:3000/admin을 입력해서 접속해야 전체 통계와 사용자가 추가한 커스텀 질문 목록을 확인할 수 있습니다.
...
- **프론트 구현**: 원하는 화면 예시를 프롬프트에 제시하여 Next.js 화면들과 Tailwind 스타일링을 AI와 반복적으로 주고받으며 완성했습니다.
